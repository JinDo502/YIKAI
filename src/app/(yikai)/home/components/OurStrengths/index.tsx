...
const Card = (props: { title: string; description: string; icon: string }) => {
  ...
  return (
    <div className='p-4 md:p-8 bg-white shadow-[0px_0px_30px_rgba(0,0,0,0.1)] flex flex-col items-start gap-4 md:gap-8 group'>
      <div className='relative'>
        <i className={`${icon} relative z-20 flex-shrink-0 text-4xl md:text-6xl text-[var(--heading)]`} />
        <div className='absolute top-1/2 -right-1/2 -translate-y-1/2 -translate-x-1/4 rounded-full h-8 w-8 md:h-12 md:w-12 bg-[var(--heading)] opacity-10 group-hover:opacity-100 group-hover:bg-[var(--accent)] group-hover:scale-110 group-hover:-translate-x-1/2 transition-all duration-300 z-10' />
      </div>
      <div className='space-y-2'>
        <h3 className='text-xl md:text-2xl font-bold text-[var(--heading)] opacity-80 inline-block after:block after:w-full after:h-1 after:bg-[var(--heading)] after:opacity-10 after:mt-1 after:md:mt-2 after:transition-all after:duration-300 group-hover:after:opacity-100 group-hover:after:bg-[var(--accent)]'>
          {title}
        </h3>
