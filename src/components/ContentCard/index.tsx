...
const ContentCard = (props: ContentCardProps) => {
    ...
    <>
      ...
      <div className={`px-3 col-span-${titlePosition !== 'center' ? '1' : '2'}`}>
        <div className={`pb-15 flex flex-col items-${titlePosition} justify-center font-roboto`}>
          <EnterAnimate type='fade' className={`relative text-3xl font-bold text-[var(--heading)] ${handleTitlePosition(titlePosition)}`}>
            {title}
          </EnterAnimate>
          {subTitle && (
  ...
  return (
    <div style={{ backgroundColor: `${backgroundColor}` }}>
      {animationProps ? (
        <EnterAnimate {...animationProps} className={`w-full h-full max-w-7xl m-auto py-15 grid grid-cols-1 md:grid-cols-2 gap-8`}>
          {contentWrapper}
        </EnterAnimate>
      ) : (
        <div className={`w-full h-full max-w-7xl m-auto py-15 grid grid-cols-1 md:grid-cols-2 gap-8`}>{contentWrapper}</div>
      )}
    </div>
  );
};
